# %%  -------------------------------------------------  single‑cell boids text optimiser  -------------------------------------------------
...
from scipy.ndimage import distance_transform_edt   # pip install scipy
...
def render_frame_fast(
    pos: torch.Tensor, r: torch.Tensor, rgb: torch.Tensor, vel: torch.Tensor | None,
    size: Tuple[int, int], bg: RGB_Int, *, window: Tuple[float, float, float, float],
    edge_margin: float = 0.10, head_frac: float = 0.55,
    head_width_frac: float = 0.55, shaft_width_frac: float = 0.25,
    ...
    if vel is not None:
        ...
        for (x, y), v, rad in zip(pos_np, vel_np, r_np):
            ...
            head = ((x - xmin) * sx + ux * diam * 0.5,
                    (ymax - y) * sy + uy * diam * 0.5)
            draw.line([tail, head], fill=(255, 255, 255, 230),
...
def default_params(**kw) -> Dict[str, float]:
    p = dict(dt=0.03, neighbor_dist=2.0, separation_dist=0.25,
             cohesion_weight=1.0, alignment_weight=1.0, separation_weight=1.5,
             max_speed=1.2, max_force=1.5, world_half_size=8.0)
    p.update(kw)
    return p
...
def step(state: Dict[str, torch.Tensor], p: Dict[str, float], *, wrap: bool = True
    ...
    acc = coh + ali + sep
    acc = acc / acc.norm(dim=1, keepdim=True).clamp(min=1e-12) * \
          acc.norm(dim=1, keepdim=True).clamp(max=fmax)

    vel = vel + acc * dt
    vel = vel / vel.norm(dim=1, keepdim=True).clamp(min=1e-12) * \
          vel.norm(dim=1, keepdim=True).clamp(max=vmax)
...
def rollout(init: Dict[str, torch.Tensor], p: Dict[str, float], steps: int
    ...
    traj = [init]
    for _ in range(steps): traj.append(step(traj[-1], p))
    return traj

# ───────────── text utilities ─────────────
def make_text_mask(text: str, canvas_px: int = 256, fill_frac: float = 0.4,
    ...
    try: w, h = draw.textsize(text, font=font)
    except AttributeError:
        box = draw.textbbox((0, 0), text, font=font); w, h = box[2] - box[0], box[3] - box[1]
...
def text_sdf(text: str, canvas_px: int = 512, fill_frac: float = 0.35,
             font_path: str | None = None) -> torch.Tensor:
    mask = make_text_mask(text, canvas_px, fill_frac, font_path).squeeze()
...
def sdf_loss(pos: torch.Tensor, sdf: torch.Tensor, half: float,
             margin: float = 0.02) -> torch.Tensor:
    g = pos.clone()
    g[:, 0] = ( g[:, 0] / half).clamp(-1, 1)
    g[:, 1] = (-g[:, 1] / half).clamp(-1, 1)
    d = torch.nn.functional.grid_sample(sdf, g.view(1, -1, 1, 2),
                                        align_corners=True).view(-1)
    return torch.relu(d + margin).mean()

def spread_loss(pos: torch.Tensor, half: float) -> torch.Tensor:
    d = pos[:, None] - pos[None]
    dist = d.norm(dim=-1) + 1e-8
    m = (dist > 0).float()
    ideal = (2 * half / pos.size(0)) ** 0.5
    return (torch.relu(ideal - dist) * m).sum() / m.sum().clamp(1)

# ───────────── optimiser ─────────────
def optimise_state_to_text(N: int, p: Dict[str, float], *, text: str = "HELLO",
                           steps: int = 300, iters: int = 1000, lr: float = 0.3,
                           preview: int = 100, canvas_px: int = 512,
                           fill_frac: float = 0.35, device: str = "cpu"
                           ) -> Dict[str, torch.Tensor]:
    half = p["world_half_size"]
    mask_bin = make_text_mask(text, canvas_px, fill_frac).to(device)
    sdf = text_sdf(text, canvas_px, fill_frac).to(device)
    mask_overlay = (mask_bin, (0, 255, 0), 0.35)
    ...
    for it in (bar := tqdm(range(iters), desc="optimising")):
        ...
        s = state
        for _ in range(steps): s = step(s, p, wrap=False)

        loss = sdf_loss(s["pos"], sdf, half) + 0.01 * spread_loss(s["pos"], half)
        ...
        if it % preview == 0 or it == iters - 1:
            frame = render_frame_fast(
                s["pos"], r, rgb, s["vel"], (600, 600), (0, 0, 0),
                window=(-half, half, -half, half), mask=mask_overlay)
...
if __name__ == "__main__":
    ...
    with timer("optimise to text"):
        state = optimise_state_to_text(N, params, text="Hi!",
                                       steps=STEPS, iters=ITERS, lr=0.1,
                                       preview=PREVIEW, canvas_px=768,
                                       fill_frac=0.32, device=DEV)

    traj = rollout(state, params, steps=STEPS)
    frames = [
        render_frame_fast(s["pos"], s["r"], s["rgb"], s["vel"],
                          (600, 600), (0, 0, 0),
                          window=(-params["world_half_size"], params["world_half_size"],
                                  -params["world_half_size"], params["world_half_size"]))
        for i, s in enumerate(traj) if (i % 5 == 0 or i == len(traj) - 1)
    ]
